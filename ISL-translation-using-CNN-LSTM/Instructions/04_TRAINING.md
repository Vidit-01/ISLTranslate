...
data:
  npy_dir: /content/drive/MyDrive/isl_project/data/keypoints/
  splits_dir: /content/drive/MyDrive/isl_project/data/splits/
  target_len: 64
  augment_train: true

checkpoint:
  dir: /content/drive/MyDrive/isl_project/checkpoints/bilstm/
  save_every: 10
  save_best: true
```
...
checkpoint:
  dir: /content/drive/MyDrive/isl_project/checkpoints/spoter/
  save_every: 10
  save_best: true
```
...
**Overfitting is expected** on INCLUDE (~17 videos/class). Mitigations in order of impact:
1. Augmentation (already in pipeline)
2. Dropout ≥ 0.1 in SPOTER, ≥ 0.3 in BiLSTM
3. Label smoothing 0.1
4. Weight decay 1e-4
5. If still overfitting: reduce `num_encoder_layers` to 4, or `d_model` to 128
6. If all else fails: pretrain on WLASL, fine-tune with lr=1e-5

**Expected training time on T4:**
- BiLSTM: ~2 min/epoch → 80 epochs ≈ 2.5 hours
- SPOTER: ~4 min/epoch → 150 epochs ≈ 10 hours (use Colab Pro or run in two sessions, save checkpoints)
